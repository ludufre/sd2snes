...
parameter ST_SMS_RD_ADDR = 15'b000100000000000;
parameter ST_SMS_RD_END  = 15'b001000000000000;
parameter ST_SMS_WR_ADDR = 15'b010000000000000;
parameter ST_SMS_WR_END  = 15'b100000000000000;
...
// SMS core (M7.4b)
wire SMS_RD_HIT = |(STATE & (ST_SMS_RD_ADDR | ST_SMS_RD_END));
wire SMS_WE_HIT = |(STATE & ST_SMS_WR_ADDR);
wire SMS_WR_HIT = |(STATE & (ST_SMS_WR_ADDR | ST_SMS_WR_END));
wire SMS_HIT = SMS_RD_HIT | SMS_WR_HIT;

wire        sms_rom_rrq;   wire [23:0] sms_rom_addr;
wire        sms_buf_wrq;   wire [23:0] sms_buf_addr;  wire [7:0] sms_buf_data;
wire        sms_frame_done;
reg         SMS_RD_PENDr = 1'b0, SMS_WR_PENDr = 1'b0;
reg  [23:0] SMS_ROM_ADDRr = 24'h0, SMS_BUF_ADDRr = 24'h0;
reg  [7:0]  SMS_DINr = 8'h0, SMS_BUF_DATAr = 8'h0;
reg         sms_rom_rdy = 1'b0, sms_buf_wack = 1'b0;
wire [23:0] SMS_ADDR_MUX = SMS_WR_HIT ? SMS_BUF_ADDRr : SMS_ROM_ADDRr;

// SNES pad -> SMS controller $DC. The player writes SNES auto-joypad JOY1 to
// $EF:0000 (JOY1L) / $EF:0001 (JOY1H); we snoop the write and map to $DC
// (active-low: bit0..5 = Up Down Left Right Button1 Button2).
reg [15:0] sms_pad_raw = 16'h0;
always @(posedge CLK2)
  if (SNES_WR_end & (SNES_ADDR[23:16] == 8'hEF) & (SNES_ADDR[7:1] == 7'd0)) begin
    if (~SNES_ADDR[0]) sms_pad_raw[7:0]  <= SNES_DATA;   // $EF0000 JOY1L
    else               sms_pad_raw[15:8] <= SNES_DATA;   // $EF0001 JOY1H
  end
wire [7:0] sms_pad1 = ~{2'b00,
   (sms_pad_raw[14] | sms_pad_raw[7]),   // Button2 = SNES Y|A
   (sms_pad_raw[15] | sms_pad_raw[7]),   // Button1 = SNES B|A (jump)
   sms_pad_raw[8],                       // Right
   sms_pad_raw[9],                       // Left
   sms_pad_raw[10],                      // Down
   sms_pad_raw[11]};                     // Up

// double-buffer with a ready handshake. The FPGA writes the BACK set; on FRAME_DONE
// the back is complete (sms_ready=1). The player writes $EF0005 at NMI END (after its
// DMA); if a buffer is ready, swap (front<->back) + clear ready. TR_GATE holds off new
// translations until the player consumes the back (no overwrite-while-read race).
reg sms_buf_front = 1'b0;
reg sms_ready     = 1'b0;
wire sms_swap = SNES_WR_end & (SNES_ADDR[23:16] == 8'hEF) & (SNES_ADDR[7:0] == 8'h05);
always @(posedge CLK2) begin
  if (sms_swap & sms_ready)      sms_buf_front <= ~sms_buf_front;
  if (sms_frame_done)            sms_ready <= 1'b1;          // new back complete (wins)
  else if (sms_swap & sms_ready) sms_ready <= 1'b0;          // player consumed it
end

// SMS clock-enable = EXACT 3.579545MHz average via a fractional-N accumulator
// (was /27 = 3.556MHz = -0.67% game speed; /24 was 4.0MHz = ~67Hz frames).
// K = round(2^25 * 3579545 / 96e6) = 1251194; carry-out = CE. CE spacing dithers
// 26/27 CLK2 cycles (>=26), so the tv80 cen multicycle-slack analysis still holds.
reg [24:0] sms_ce_acc = 25'd0; reg sms_ce = 1'b0;
always @(posedge CLK2) begin
  {sms_ce, sms_ce_acc} <= {1'b0, sms_ce_acc} + 26'd1251194;
end
reg sms_wr_busy = 1'b0;   // double-issue guard: block re-latch until BUF_WRQ drops

// SMS request latches; the STATE FSM services them on free slots
always @(posedge CLK2) begin
  sms_rom_rdy <= 1'b0; sms_buf_wack <= 1'b0;
  if (sms_rom_rrq & ~SMS_RD_PENDr) begin
    SMS_RD_PENDr  <= 1'b1;
    SMS_ROM_ADDRr <= 24'h300000 + sms_rom_addr;
  end else if (|(STATE & ST_SMS_RD_END)) begin
    SMS_RD_PENDr <= 1'b0; sms_rom_rdy <= 1'b1;
  end
  if (sms_buf_wrq & ~SMS_WR_PENDr & ~sms_wr_busy) begin
    SMS_WR_PENDr  <= 1'b1;
    SMS_BUF_ADDRr <= (sms_buf_front ? 24'h180000 : 24'h190000) + sms_buf_addr;  // write the back set
    SMS_BUF_DATAr <= sms_buf_data;
  end else if (|(STATE & ST_SMS_WR_END)) begin
    SMS_WR_PENDr <= 1'b0; sms_buf_wack <= 1'b1; sms_wr_busy <= 1'b1;
  end
  if (sms_wr_busy & ~sms_buf_wrq) sms_wr_busy <= 1'b0;  // sms_core saw the ACK + dropped BUF_WRQ
end

sms_core sms_inst (
  .CLK(CLK2), .RST(SNES_DEADr), .CE(sms_ce),
  .ROM_RRQ(sms_rom_rrq), .ROM_ADDR(sms_rom_addr), .ROM_DATA(SMS_DINr), .ROM_RDY(sms_rom_rdy),
  .BUF_WRQ(sms_buf_wrq), .BUF_ADDR(sms_buf_addr), .BUF_DATA(sms_buf_data), .BUF_WACK(sms_buf_wack),
  .PAD1(sms_pad1), .FRAME_DONE(sms_frame_done),
  .DBG_PC(), .DBG_VRAM_WRITES(), .DBG_DISPLAY_ON(), .DBG_TR_BUSY(),
...
assign ROM_ADDR  = (SD_DMA_TO_ROM) ? MCU_ADDR[23:1] : CTX_HIT ? CTX_ROM_ADDRr[23:1] : DMA_HIT ? DMA_ROM_ADDRr[23:1] : MCU_HIT ? ROM_ADDRr[23:1] : SMS_HIT ? SMS_ADDR_MUX[23:1] : MAPPED_SNES_ADDR[23:1];
assign ROM_ADDR0 = (SD_DMA_TO_ROM) ? MCU_ADDR[0]    : CTX_HIT ? CTX_ROM_ADDRr[0]    : DMA_HIT ? DMA_ROM_ADDRr[0]    : MCU_HIT ? ROM_ADDRr[0]    : SMS_HIT ? SMS_ADDR_MUX[0] : MAPPED_SNES_ADDR[0];
...
wire ROM_ADDR22;
assign ROM_ADDR22 = (SD_DMA_TO_ROM) ? MCU_ADDR[1]    : CTX_HIT ? CTX_ROM_ADDRr[1]    : DMA_HIT ? DMA_ROM_ADDRr[1]    : MCU_HIT ? ROM_ADDRr[1]    : SMS_HIT ? SMS_ADDR_MUX[1] : MAPPED_SNES_ADDR[1];
assign ROM_ADDR   = (SD_DMA_TO_ROM) ? MCU_ADDR[23:2] : CTX_HIT ? CTX_ROM_ADDRr[23:2] : DMA_HIT ? DMA_ROM_ADDRr[23:2] : MCU_HIT ? ROM_ADDRr[23:2] : SMS_HIT ? SMS_ADDR_MUX[23:2] : MAPPED_SNES_ADDR[23:2];
assign ROM_ADDR0  = (SD_DMA_TO_ROM) ? MCU_ADDR[0]    : CTX_HIT ? CTX_ROM_ADDRr[0]    : DMA_HIT ? DMA_ROM_ADDRr[0]    : MCU_HIT ? ROM_ADDRr[0]    : SMS_HIT ? SMS_ADDR_MUX[0] : MAPPED_SNES_ADDR[0];
  ...
  case(STATE)
    ST_IDLE: begin
      ...
      if(free_slot | SNES_DEADr) begin
        ...
        else if(SMS_RD_PENDr) begin
          STATE <= ST_SMS_RD_ADDR;
          ST_MEM_DELAYr <= ROM_CYCLE_LEN;
        end
        else if(SMS_WR_PENDr) begin
          STATE <= ST_SMS_WR_ADDR;
          ST_MEM_DELAYr <= ROM_CYCLE_LEN;
        end
      end
    end
    ...
    ST_SMS_RD_ADDR: begin
      STATE <= ST_SMS_RD_ADDR;
      ST_MEM_DELAYr <= ST_MEM_DELAYr - 1;
      if(ST_MEM_DELAYr == 0) STATE <= ST_SMS_RD_END;
      SMS_DINr <= (ROM_ADDR0 ? ROM_DATA[7:0] : ROM_DATA[15:8]);
    end
    ST_SMS_WR_ADDR: begin
      STATE <= ST_SMS_WR_ADDR;
      ST_MEM_DELAYr <= ST_MEM_DELAYr - 1;
      if(ST_MEM_DELAYr == 0) STATE <= ST_SMS_WR_END;
    end
    ST_MCU_RD_END, ST_MCU_WR_END, ST_CTX_WR_END, ST_DMA_RD_END, ST_DMA_WR_END,
    ST_SMS_RD_END, ST_SMS_WR_END: begin
      STATE <= ST_IDLE;
    end
  endcase
end
