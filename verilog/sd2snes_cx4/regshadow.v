...
// `pawr_end` is wired (see main.v) to the raw /PAWR rising edge; `snes_pa`/`snes_data`
// are the register offset and byte latched from shallow 1-cycle taps during the pulse.
// On the 80 MHz CX4 core the /PAWR pulse is only ~4 cycles, so the deep SNES_PA
// debounce and any cycle-count strobe overshot; the shallow raw-pulse-aligned capture
// grabs the current store's PA+byte together, before the next store.
