...
// a26 framebuffer read window: the SNES reads the set the core is NOT writing
// from banks $E0-$E3 -> PSRAM 0x380000 (set A) / 0x390000 (set B), stride
// 0x10000.  The banks are sugar over one set (16-bit bank offset), so an
// offset must never cross the stride:
//  $E0=tiles(0x0000) $E1=reserved(0x3000) $E2=control+HDMA(0x4000) $E3=reserved(0x6000)
// READ side polarity: front=1 -> the SNES reads set B (0x390000) while the
// core writes set A; front=0 -> the other way round.  The WRITE path adds
// 0x180000/0x190000 plus the wrapper's BUF_BASE 0x200000 in main.v -- keep in
// sync with main.v.
wire A26_BUF_HIT = (SNES_ADDR[23:18] == 6'b111000);   // banks $E0..$E3
wire [23:0] a26_sel_off = (SNES_ADDR[17:16] == 2'b00) ? 24'h00000
                        : (SNES_ADDR[17:16] == 2'b01) ? 24'h03000
                        : (SNES_ADDR[17:16] == 2'b10) ? 24'h04000
                                                      : 24'h06000;
wire [23:0] a26_set_base = a26_buf_front ? 24'h390000 : 24'h380000;
wire [23:0] A26_BUF_ADDR = a26_set_base + a26_sel_off + {8'd0, SNES_ADDR[15:0]};

assign ROM_ADDR = A26_BUF_HIT ? A26_BUF_ADDR : SRAM_SNES_ADDR;

assign ROM_HIT = A26_BUF_HIT | IS_ROM | IS_WRITABLE | bs_page_enable;
