...
//
// Vendored for sd2snes_nes (Phase -1), except:
// 1. Header only: the upstream `include "MicroCode.v"` was removed
//    (MicroCode.v is compiled as its own translation unit via the Makefile
//    VSRC list -- see nes.v's header for why).
// 2. FUNCTIONAL FIX (found by simulation, not by inspection -- see the
//    Phase -1 report for the full derivation): every `casez` in this file,
//    plus every plain `case` whose selector is a field decoded straight out
//    of the MicroCode/Mout ROMs (ALCtrl/AHCtrl/LoadPC/StateCtrl/MemWrite/
//    AddrBus/LoadSP/LoadT/FlagsCtrl, and NewAlu's o_left/o_right/o_first_op/
//    o_second_op), is now `casex`. MicroCode.v's A[]/B[]/L[] tables encode
//    genuine "don't care" control bits as literal `x` for slots the author
//    knew were behaviorally equivalent either way (e.g. B[8'h A2]'s
//    o_second_op = 3'b10x for LDX #imm: cases 4 and 5 both produce the exact
//    same `{CO,Result} = {CR,IntR}` passthrough, so it truly doesn't matter
//    which one an X bit "becomes"). On real silicon that X is some concrete
//    voltage that still lands on one of the two equivalent branches. In a
//    4-state RTL simulator, `casez` only wildcards `z` -- an `x` selector bit
//    matches NEITHER concrete case item, so the `casez` falls through with no
//    branch taken and the target reg (here, `Result`/`IntR`/`LoadPC`'s
//    `NewPC`/etc.) silently keeps its stale prior value instead of updating.
//    Symptom that led here: `LDX #$00` (the very first LDX nestest executes)
//    loaded X with a stale leftover ALU result instead of 0 -- traced via
//    hierarchical probes of cpu.new_alu.o_second_op down to exactly this
//    dead branch. `casex` (which wildcards `x` too) makes the simulated
//    behavior match what happens on real hardware: any of the equivalent
//    branches is selected, and since the code already only lists an X bit in
//    positions where every matching branch is behaviorally identical, this
//    changes nothing else. Left `case (DIN[7:5])` (branch-condition decode,
//    line ~345) as plain `case`: it selects on the live opcode byte read off
//    the bus, never on an X-bearing ROM field, so it was never at risk and
//    should stay exact-match.
...
module CPU(input clk, input ce, input reset,
           ...
           output reg mr,
           output reg mw,
           // sd2snes: debug snapshot exported as a real port (Quartus cannot
           // synthesize hierarchical references) = {IsResetInterrupt, State,
           // P, SP, Y, X, A, PC}
           output [59:0] dbg_out);
  reg [7:0] A, X, Y;
  reg [7:0] SP, T, P;
  ...
  wire [15:0] PC;
  assign dbg_out = {IsResetInterrupt, State, P, SP, Y, X, A, PC};
  reg JumpTaken;
  wire JumpNoOverflow;
  ...
  // NOTE: AX/AXCarry moved up from their original position (just above the
  // AddressGenerator instantiation below) to before their first use in the
  // NextState `always @*` block. Icarus Verilog 13.0 requires a wire read
  // inside a procedural block to be declared earlier in the file than that
  // read ("Check for declaration after use"); Quartus/other tools that
  // resolve the whole module scope before elaborating do not care about
  // this ordering, so this is a pure portability fix, not a functional one.
  wire [15:0] AX;
  wire AXCarry;
