// Vendored verbatim from strigeus/fpganes src/MicroCode.v (auto-generated
// microcode ROM tables, no logic changes) for sd2snes_nes (Phase -1), EXCEPT:
// every `<size>'b_<bits>` literal had its leading underscore (immediately after
// the base specifier) stripped to plain `<size>'b<bits>` -- IEEE 1364-2005
// SS3.5.1 disallows `_` as the first character of the value part, and while some
// toolchains silently accept it, Icarus Verilog 13.0 rejects it outright
// ("Malformed statement"). Underscores elsewhere in the same literals (e.g.
// `10__0_10101`) are untouched -- those are mid-literal and always legal.
