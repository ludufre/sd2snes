...
// DEVIATION from upstream fpganes (documented): the tmp_a/tmp_b output
// registers of this table were the ONLY sequential elements in the whole NES
// hierarchy NOT gated by `ce` (upstream captures them on every raw clk edge).
// That breaks the invariant the sd2snes port's timing closure is built on --
// main.sdc declares multicycle constraints for ALL reg->reg paths inside the
// NES on the argument that every capture happens on a ce edge (ce pulses are
// >= 13 CLK2 cycles apart, see nes_wrap.v). A free-running capture flop under
// that relaxed constraint would latch mid-flight garbage. Gating the capture
// by `ce` (port added, wired to the APU's own `ce`) restores the invariant;
// the only behavioral difference is the mixed audio sample updating one
// dot-clock (~186ns) later than upstream -- inaudible, and the audio path is
// not validated in this phase anyway.
module ApuLookupTable(input clk, input ce, input [7:0] in_a, input [7:0] in_b, output [15:0] out);
