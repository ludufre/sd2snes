...
// Description:
//   Small write-only register shadow for the in-game cheat overlay on the mk2
//   SA-1 core (Spartan-3).  The overlay restores the SNES PPU ($2100-$213F) and
//   CPU ($4200-$421F) registers from a shadow of the last value written to each,
//   read back through the NMI hook's identity window as PSRAM:
//     $F90500-$F9057F : PPU regs, stride-2 words (low byte = value, high = $00)
//     $F90700-$F9071F : CPU $42xx regs, stride-1 bytes
//   On mk3 that shadow is a by-product of the full ctx.v PSRAM mirror (ctx writes
//   $F90500 + PA*2 and $F90700 + addr[8:0]); ctx is far too large for the mk2
//   core, so this replaces just the one piece the overlay needs with a single
//   RAMB16 (13 of 16 Block RAMs are free on mk2, LUTs are the scarce resource).
//
...
wire       cpu_wr = wr_end & (snes_addr[23:16] == 8'h00)
