...
//   On the SA-1 mk3 core that shadow is a by-product of the full ctx.v PSRAM
//   mirror (ctx writes $F90500 + PA*2 and $F90700 + addr[8:0]); ctx is far too
//   large for the mk2 Spartan-3 SA-1 core, so this replaces just the one piece
//   the overlay needs with a single RAMB16 (13 of 16 Block RAMs are free on mk2;
//   LUTs are the scarce resource).  Logically identical to the module of the same
//   name on gsu/cx4/obc1/sdd1 -- only the wiring comments below are per-core.
//
...
// Per-core wiring (see the instance in main.v): on this core `pawr_end` is the
// core's own settled /PAWR rising-edge strobe SNES_PAWR_end (SNES_PAWRr[6:1] ==
// 6'b000001, a one-cycle pulse) and `snes_data` is the settled tap SNES_DATA_IN
// -- this is the exact capture that REGSHADOW_TEST validated in hardware on the
// SA-1 core at CLK2 = 85.9 MHz, so it is kept as-is.  (The gsu/cx4/obc1/sdd1
// cores lack that strobe and rebuild a ctx-style counter, rs_pawr_end_r, for the
// same purpose.)  Both bytes of a pair are sampled on the strobe cycle, so the
// 2-deep scheme does not change this capture timing.
...
// Undefined everywhere else -- including THIS core on both targets, and every mk3
// core -- where the preprocessed source is identical to the ungated version.
