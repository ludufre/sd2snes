...
uint32_t bps_apply(uint32_t sram_addr, uint8_t index, uint32_t rom_base_addr,
    ...
    /* Copier mode (Approach B+): the descriptor list is built in PSRAM right above
       the backup region [source_base, +orig); reset before the backup. */
    patch_targetread_bytes = 0;
    if (use_copier) patch_copier_reset(source_base_addr + original_rom_size);
