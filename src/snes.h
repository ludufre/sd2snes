...
#define SNES_CMD_MANUAL_BLOCK        (0x43) /* in-game guides viewer: stage a guide block into PSRAM SRAM_MANUAL_BLOCK_ADDR. MCU_PARAM[0..1] = stream block index (LE), [2] = compacted guide (0..7), [3] = mode (bit0 = zoom stream); ACK = the standard snes_set_mcu_cmd(0) clear. Bounded (one SD read); lockstep with CMD_MANUAL_BLOCK in snes/memmap.i65 */
