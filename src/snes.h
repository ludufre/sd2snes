...
#define SNES_CMD_HOT_RELOAD          (0x39) /* SYNTHETIC (never sent by the SNES): menu_main_loop returns this when the Companion asked for a hot-reload (UP_OP_HOT_RELOAD) -> main.c sets menu_reload so the menu re-reads config.yml/theme/lists without a power-cycle. Was 0x38 (moved to 0x39: GI_DESC_FULL took 0x38 from feat-pre-launch-info). Synthetic, so the value is arbitrary. */
