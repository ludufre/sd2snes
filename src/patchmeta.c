...
void patchmeta_save(const uint8_t *rom_path, uint32_t sram_addr, uint8_t count) {
  ...
  /* Shared with cheat_yaml_save: bucket mkdir (WRITE path only, and only once
     the name is built), attribute clear + unlink, open with the truncate
     fallback, and the document header. */
  if(yaml_open_write(pm_path)) {
    file_res = 0;
    return;
  }
  ...
  for(uint8_t i = 0; i < count; i++) {
    ...
    yaml_put_quoted(&file_handle, "- Patch: \"", pm_leaf);
    yaml_put_quoted(&file_handle, "  Name: \"", cur);
