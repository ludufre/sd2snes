...
_Static_assert(sizeof(spc7110_rtc_file_t) == 24, "spc7110 rtc sidecar layout");
...
void spc7110_rtc_load(uint8_t *filename) {
  ...
  if (path_asset(rtcfile, sizeof(rtcfile), SAVE_BASEDIR, (const char *)filename,
                 ".rtc") < 0) { spc7110_rtc_wallclock(); return; }
...
void spc7110_rtc_save(uint8_t *filename) {
  ...
  if (path_asset(rtcfile, sizeof(rtcfile), SAVE_BASEDIR, (const char *)filename,
                 ".rtc") < 0) return;
  path_asset_mkdir(rtcfile);            /* only once the name exists */
