...
/* Approach B: patch_copier_emit records each SourceCopy/TargetCopy op; patch_copier_
   finish replays them in order -- a forward element-by-element copy is exactly the
   FPGA copier (overlapping src<dst reproduces RLE inflate).  The source backup runs
   immediately via patch_copier_op_now (must read pristine source before run_actions).
   Deferred replay is byte-identical because the inline TargetReads are already
   applied and the copier ops replay in action order.  host_copier_replay() is a
   no-op (finish already did the work; kept so the CLI/harness call sites stay
   unchanged). */
struct hc_op { uint32_t src, dst, len; };
static struct hc_op *hc_ops;
static uint32_t hc_n, hc_cap;

void patch_copier_reset(void) { hc_n = 0; }
