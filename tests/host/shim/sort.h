/* Host shim for src/sort.h: same prototypes, minus the `#include CONFIG_MCU_H`
 * the real header carries (that macro only exists once autoconf.h -- the
 * per-board generated header -- has run; the host build has no board). */
#ifndef HOST_SORT_H
#define HOST_SORT_H
